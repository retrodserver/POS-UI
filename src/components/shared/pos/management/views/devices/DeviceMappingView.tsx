...
export function DeviceMappingView() {
  const devices = [
    { id: "dev-1", name: "Billing Terminal 1 (Windows POS)", type: "Desktop", ip: "192.168.1.101", status: "Online" },
    { id: "dev-2", name: "Captain Tablet 1 (Samsung Tab A9)", type: "Tablet", ip: "192.168.1.115", status: "Online" },
    { id: "dev-3", name: "Captain Tablet 2 (iPad 9th Gen)", type: "Tablet", ip: "192.168.1.118", status: "Online" },
    { id: "dev-4", name: "Kitchen Display System (KDS Screen)", type: "Display", ip: "192.168.1.130", status: "Online" },
  ];
    ...
    <div className="space-y-4 max-w-4xl">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-[12.5px] text-slate-500 mt-0.5">
            Authorized POS hardware, Captain Android/iOS tablets, and kitchen display screen bindings.
          </p>
        </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white shadow-xs overflow-hidden">
        <div className="divide-y divide-slate-100">
          {devices.map((dev) => (
            <div key={dev.id} className="p-4 flex items-center justify-between hover:bg-slate-50/50 transition">
              <div className="flex items-center gap-3">
                <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-teal-50 text-teal-600">
                  {dev.type === "Desktop" ? <Monitor className="h-5 w-5" /> : <Tablet className="h-5 w-5" />}
                </div>
                <div>
                  <div className="font-bold text-[13.5px] text-slate-900">{dev.name}</div>
                  <div className="text-[12px] font-mono text-slate-500">IP: {dev.ip} • Type: {dev.type}</div>
                </div>
              </div>
